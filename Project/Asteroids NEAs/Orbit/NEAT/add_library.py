...
def labelNEAT(new):
    ...
    time = []
    for idx in np.arange(len(new)):
        if (new['frame'].iloc[idx] != new['frame'].iloc[idx - 1]) and (
                new['date'].iloc[idx] != new['date'].iloc[idx - 1]):
            timenum = 0
        elif (new['frame'].iloc[idx] == new['frame'].iloc[idx - 1]) and (
                new['date'].iloc[idx] == new['date'].iloc[idx - 1]):
            timenum = time[idx - 1]
        elif (new['frame'].iloc[idx] == new['frame'].iloc[idx - 1]) and (
                new['date'].iloc[idx] != new['date'].iloc[idx - 1]):
...
def removeStars(filename, df_allSources, StarFile):
    ...
    if os.path.exists('inputs / % s_dfStarsRemoved.pickle' % filename):
        with open('nputs / % s_dfStarsRemoved.pickle' % filename, 'rb') as f:
...
def tblConvert(filename, StarFile):
    ...
    if StarFile:
        ...
        data['name'] = df['sid']
        data['date'] = df['obsmjd'].apply(lambda x: datetime.datetime.strptime(Time(x, format='mjd', out_subfmt
        ='date*').iso, '%Y-%m-%d%H:%M:%S.%f'))

        data['RA'] = df['ra'].apply(
            lambda x: Angle(x, u.degree).to_string(unit=u.hourangle, sep='', precision=2, fields=3))

        data['Dec'] = df['dec'].apply(
            lambda x: Angle(x, u.degree).to_string(unit=u.degree, sep='', precision=1, alwayssign=True, fields=3))
        ...
        for idx in np.arange(len(data)):
            if idx == 0:  # this needs to be added in NEAT library , in label_NEAT function
                timenum = 0
            elif (data['frame'].iloc[idx] != data['frame'].iloc[idx - 1]) and (
                    data['date'].iloc[idx] != data['date'].iloc[idx - 1]):
                timenum = 0
            elif (data['frame'].iloc[idx] == data['frame'].iloc[idx - 1]) and (
                    data['date'].iloc[idx] == data['date'].iloc[idx - 1]):
                timenum = times[idx - 1]
        ...
        for idx in np.arange(len(data)):
            RA = Angle(df['ra'].iloc[idx], u.degree).to_string(unit=u.hourangle, sep='', pad=True,
                                                               precision=2, fields=3)
            Dec = Angle(df['dec'].iloc[idx], u.degree).to_string(unit=u.degree, sep='', pad=True,
                                                                 precision=1, alwayssign=True, fields=3)
            date = datetime.datetime.strptime(Time(df['obsmjd'].iloc[idx], format='mjd',
                                                   out_subfmt='date*').iso, '%Y-%m-%d%H:%M:%S.%f')
...
def addNoise(df, noise_lvl, maxSep, filename, lines):
    ...
    are appended at the end of file .
    """
    ...
    f = open("%s.txt" % filename, "a")
    maxIm = df['image'].max()
    for i in np.arange(numNoise):  # generate noise elements
        image = random.randint(0, maxIm)
        field = df[df.image == image]
        field = field[field.line < maxLines]
        template = random.randint(0, len(field) - 1)
        templateCoord = SkyCoord(ra=field['RA'].iloc[template], dec=field['Dec'].iloc[template],
        ...
        df = df.append({'name': 'FAKE' + field['name'].iloc[template][4:7],
                        ...
                        'time': field['time'].iloc[template]}, ignore_index=True)
