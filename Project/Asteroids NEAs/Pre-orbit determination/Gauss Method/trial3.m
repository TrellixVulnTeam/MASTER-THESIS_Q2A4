...
% compute the state vector from the data provided in
% Example 5.11.
%
...
% ra - vector of topocentric equatorial right
% ascensions at t1, t2, t3 (deg)
% dec - vector of topocentric equatorial right
% declinations at t1, t2, t3 (deg)
% theta - vector of local sidereal times for t1, t2, t3
% (deg)
% R - matrix of site position vectors at t1, t2, t3
% (km)
% rho - matrix of direction cosine vectors at t1,
% t2, t3
% fac1, fac2 - common factors
% r_old, v_old - the state vector without iterative improvement
% (km, km/s)
...
phi = 40*deg;
%t = [0 148.2 294.6];
%ra = [227.2792 227.7375 227.1083];
%dec = [10.0814 10.08172 10.0827];
%theta = [134  135 136]; %[20.1644 20.7195 21.1669]; %  8.5925 8.8311 9.0233
t = [ 0 118.104 237.577];
...
fac2 = (Re*(1-f)^2/sqrt(1-(2*f - f*f)*sin(phi)^2) + H) ...
*sin(phi)
for i = 1:3
...
% the command window:
fprintf('---------------------------------------------------')
fprintf('\n Orbit determination by the Gauss method\n')
fprintf('\n Radius of earth (km) = %g', Re)
fprintf('\n Flattening factor = %g', f)
fprintf('\n Gravitational parameter (kmˆ3/sˆ2) = %g', mu)
fprintf('\n\n Input data:\n');
fprintf('\n Latitude (deg) = %g', phi/deg);
fprintf('\n Altitude above sea level (km) = %g', H);
fprintf('\n\n Observations:')
fprintf('\n Time (s) Right ascension (deg) Declination (deg)')
fprintf(' Local sidereal time (deg)')
